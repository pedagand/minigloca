...
\pierre{Ce n'est pas très élégant de faire des captionof : il vaudrait
  mieux faire des figure avec label et mettre des ref.}
...
n'ont pas besoin d'évaluer la déclaration pour se construire. \pierre{Je ne comprends pas :}On notera aussi que la routine peut, pour le moment, être
...
\begin{example}
	...
	\end{lstlisting}
\end{example}
L'implémentation de tout ce qui est spécifié dans ce document, est écrite en OCaml et est disponible sur le dépôt Git\pierre{mettre le lien en footnote}.
...
Voici \pierre{Où ?} un exemple d'AST sur notre langage.
...
Dans ce cadre là, on définit l'état d'un programme (sur la pile \pierre{quelle pile?}) par une application entre les identifiants de variables et leurs valeurs, 
...
\end{align*}
On définit $\mathbb{B} = \{0, 1\}$. De la même manière, on définit l'application qui suit sur les expressions booléennes, \pierre{Retirer les ``:'' comme j'ai fait au-dessus, dans toutes les définitions qui suivent.}
\begin{align*}
...
\pierre{You've changed the style definition compared to section 3.1}
\begin{align*}
...
\\
\\
Notre langage étant un langage impératif, il est par conséquent naturel d'effectuer cette analyse de bas en haut\pierre{``bas en haut'' introduit inutilement de la confusion, il ne faut pas biaiser la lecture du rapport avec une orientation des règles : c'est un système d'équation, on s'en fiche de l'ordre de résolution} dans le parcours des déclarations,
...
On les définira comme tels, \pierre{Il ne faut pas écrire $LIVE$ (l'espaçage est délirant) mais $\textit{LIVE}$ et donc définir une macro qui va bien.}
...
algorithme est le point fixe minimal. \pierre{il y a aussi (et surtout) la question de terminaison.}
...
bloc altéré. L'on décrira ce second algorithme plus bas dans la section. \pierre{Parler de ``work-list algorithm'' / ``Kildall algorithm''}
\\
\\
\begin{lemma}
...
négatifs. En effet, on aimerait plutôt avoir quelque chose comme,\pierre{je ne suis pas convaincu par ces virgules avant un bout de code}
\begin{lstlisting}[tabsize=2]
	div:
		mov     eax, DWORD PTR [rbp-4]
		shr     eax, 5
...
\pierre{Not sure about that but this cannot hurt:}
