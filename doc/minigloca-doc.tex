...
\begin{proof}
	...
	singleton. On considère $\mu_P$ le point fixe minimal sur le programme $P$.
		Si pour
	...
	Revenons-en à la construction de $\livein{\cdot}$. On a que $\forall u \in I_l[\mu_P]$ et $\forall k \in \mathbb{N}$ alors $u \notin gen[k]$ et
	$u \notin kill[k]$. En effet, ces derniers restent immuables au filtre étant donné qu'ils appartiennent à un bloc toujours vivant et que l'ensemble de
	paires est construit avec leur étiquette. L'ensemble susceptible d'être altéré est donc plutôt $\liveout{k}$.
\end{proof}
\end{document}